                ...
                <div className="product-grid">
                    {plantsArray.map((category, index) => ( // Loop through each category in plantsArray
                        <div key={index}> {/* Unique key for each category div */}
                            <h1 style={{ textAlign: 'center', padding: '45px' }}>
                                ...
                                {category.plants.map((plant, plantIndex) => ( // Loop through each plant in the current category
                                    <div className="product-card" key={plantIndex}> {/* Unique key for each plant card */}
                                        ...
                                        <button
                                            ...
                                            disabled={cart.some(item => item.name === plant.name) || addedToCart[plant.name]} // Disable if plant is already in cart
                                            style={{
                                                backgroundColor: (cart.some(item => item.name === plant.name) || addedToCart[plant.name]) ? '#ccc' : undefined,
                                                cursor: (cart.some(item => item.name === plant.name) || addedToCart[plant.name]) ? 'not-allowed' : 'pointer'
                                            }}
                                        >
                                            {(cart.some(item => item.name === plant.name) || addedToCart[plant.name]) ? 'Added to Cart' : 'Add to Cart'}
                                        </button>
                                    </div>
                                ))}
                            </div>
